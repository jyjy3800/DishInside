...
public class MemberController {
    ...
    public String openLogin(Model model) {
    	model.addAttribute("status", "로그인이 필요한 페이지입니다.");
        return "member/login";
    }
    ...
    @PostMapping("/login")
    @ResponseBody
    public MemberResponse login(HttpServletRequest request, HttpServletResponse response) {
        ...
        if (loginMember != null) {
            ...
            MemberInLogIn.logout(memberId); // 중복 로그인 리스트에서 사용자 제거
            session.invalidate(); // 기존 세션 무효화
        }

        // 3. 새로운 세션에 회원 정보 저장 & 세션 유지 시간 설정
        if (member != null) {
